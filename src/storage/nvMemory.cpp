...
bool nvMemory::saveConfig(TSettings* Settings)
{
    if (init())
    {
        ...
        json[JSON_KEY_SSID] = Settings->WifiSSID;
        json[JSON_KEY_PASW] = Settings->WifiPW;
        json[JSON_KEY_LNURLWallet] = Settings->LNURLWallet;
        json[JSON_KEY_LNURLServer] = Settings->LNURLServer;
        json[JSON_KEY_LNDHUBUri] = Settings->LNDHUBuri;
        json[JSON_KEY_BOLTCOLOR] = Settings->BoltColor;
        json[JSON_KEY_BOLTEFFECT] = Settings->BoltEffect;
        ...
        if (SPIFFS.exists(JSON_CONFIG_FILE))
        {
            ...
            if (configFile)
            {
                ...
                if (!error)
                {
                    ...
                    Settings->WifiPW = json[JSON_KEY_SSID] | Settings->WifiPW;
                    Settings->LNURLWallet = json[JSON_KEY_LNURLWallet] | Settings->LNURLWallet;
                    Settings->LNURLServer = json[JSON_KEY_LNURLServer] | Settings->LNURLServer;
                    Settings->LNDHUBuri = json[JSON_KEY_LNDHUBUri] | Settings->LNDHUBuri;
                    if (json.containsKey(JSON_KEY_BOLTCOLOR))
                        Settings->BoltColor = json[JSON_KEY_BOLTCOLOR].as<int>();
                    if (json.containsKey(JSON_KEY_BOLTEFFECT))
                        Settings->BoltEffect = json[JSON_KEY_BOLTEFFECT].as<int>();
                    return true;
                }
                else
                {
